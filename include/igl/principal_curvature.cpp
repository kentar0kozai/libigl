...
public:
  ...
  // The i-th row contains the indices of the vertices that forms the i-th face
  // in ccw order
  ...
  IGL_INLINE void finalEigenStuff(int, const std::vector<Eigen::Vector3d> &,
                                  Quadric &);
...
IGL_INLINE void CurvatureCalculator::finalEigenStuff(
    int i, const std::vector<Eigen::Vector3d> &ref, Quadric &q) {
...
IGL_INLINE void
CurvatureCalculator::computeReferenceFrame(int i, const Eigen::Vector3d &normal,
...
IGL_INLINE void CurvatureCalculator::computeCurvature() {
  ...
  for (size_t i = 0; i < vertices_count; ++i) {
    ...
    if (vv.size() < 6) {
      // std::cerr << "Could not compute curvature of radius " << scaledRadius
      // << std::endl;
      continue;
    }
...
CurvatureCalculator::printCurvature(const std::string &outpath) {
  ...
  for (int i = 0; i < vertices_count; ++i) {
    of << curv[i][0] << " " << curv[i][1] << " " << curvDir[i][0][0] << " "
       << curvDir[i][0][1] << " " << curvDir[i][0][2] << " " << curvDir[i][1][0]
       << " " << curvDir[i][1][1] << " " << curvDir[i][1][2] << endl;
  }

  of.close();
}

template <typename DerivedV, typename DerivedF, typename DerivedPD1,
          typename DerivedPD2, typename DerivedPV1, typename DerivedPV2,
          typename Index>
IGL_INLINE void igl::principal_curvature(
    ...
    Eigen::PlainObjectBase<DerivedPV2> &PV2, std::vector<Index> &bad_vertices,
    unsigned radius, bool useKring) {

  if (radius < 2) {
    radius = 2;
    std::cout << "WARNING: igl::principal_curvature needs a radius >= 2, "
                 "fixing it to 2."
              << std::endl;
  }
...
IGL_INLINE void igl::principal_curvature(
    ...
    Eigen::PlainObjectBase<DerivedPD2> &PD2,
    Eigen::PlainObjectBase<DerivedPV1> &PV1,
    Eigen::PlainObjectBase<DerivedPV2> &PV2, unsigned radius, bool useKring) {
  if (radius < 2) {
    radius = 2;
    std::cout << "WARNING: igl::principal_curvature needs a radius >= 2, "
                 "fixing it to 2."
...
template void igl::principal_curvature<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &,
    unsigned int, bool);
template void igl::principal_curvature<Eigen::Matrix<double, -1, 3, 0, -1, 3>,
    ...
    unsigned int, bool);
template void
igl::principal_curvature<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                         Eigen::Matrix<int, -1, -1, 0, -1, -1>,
                         Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                         Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                         Eigen::Matrix<double, -1, -1, 0, -1, -1>,
...
igl::principal_curvature<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                         ...
                         Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                         Eigen::Matrix<double, -1, 1, 0, -1, 1>,
                         Eigen::Matrix<double, -1, 1, 0, -1, 1>, int>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &,
    std::vector<int, std::allocator<int>> &, unsigned int, bool);
#endif
